...
echo "[$(date)] INFO: Logged in to rancher successfully"

echo "[$(date)] INFO: Creating new SonarQube instance"
...
echo "[$(date)] INFO: New SonarQube instance is up, starting the merge process ..."

echo "[$(date)] INFO: Getting $APP PV names for team $TEAM..."
...
echo "[$(date)] INFO: Finding path on nfs..."
...
echo "[$(date)] INFO: Getting pod name..."
...
if [[ -z $CONF_PATH || -z $DATA_PATH || -z $EXTENSIONS_PATH || -z $LOGS_PATH || -z $PG_PATH || -z $PG_DATA_PATH ]]; then
  echo "[$(date)] ERROR: One or more nfs paths are unset, exiting script ..."
  exit 1
fi

echo "[$(date)] INFO: All paths were fetched successfully"

echo "[$(date)] INFO: Copying SonarQube files to nfs..."
...
echo "[$(date)] INFO: Copying migration scripts and database dump to nfs..."
#if [ ! -d "$NFSDIR/$PG_PATH/migration-scripts" ]; then
#echo "migration-scripts directory does not exist, creating directory ..."
echo "[$(date)] INFO: Creating migrations-scripts & backups dirctory..."
mkdir -p $NFSDIR/$PG_PATH/migration-scripts 
mkdir -p $NFSDIR/$PG_PATH/backups
#fi
...
echo "[$(date)] INFO: Running migration scripts to restore database ..."
kubectl -n sonarqube exec $POD -c sonardb -- bash -c "cd /var/lib/postgresql/migration-scripts && ./script.sh"
echo "[$(date)] INFO: Database migrated successfully"

echo "[$(date)] INFO: Cleaning up volume from migration scripts ..."
rm -rf $NFSDIR/$PG_PATH/migration-scripts

echo "[$(date)] INFO: Restarting the service ..."
...
echo "[$(date)] INFO: Rechecking readiness ..."
ansible-playbook $ROOTDIR/check-readiness.yaml --extra-vars "web_context=/sonar hostname=$HOSTNAME"
echo "[$(date)] INFO: SonarQube successfully merged, you can now access it on http://$HOSTNAME/sonar !"
